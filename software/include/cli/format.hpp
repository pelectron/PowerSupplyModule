...
template <traits::Integer T, Fmt Format = Fmt::normal,
...
template <traits::Integer T> struct DefaultFormat<T> : public Int<T> {};

template <traits::FixPoint T, std::size_t Precision = 11,
...
public:
  constexpr FormatResult operator()(std::span<char> buf, T fp) const {
    if constexpr (Format == Fmt::normal) {
      ...
      auto res = Int<int_type, Fmt::normal, UseSignForPositive>{}(buf, int_val);
      if (not res)
        return res.error;

      buf = buf.subspan(res.size_written);
      ...
      return Int<raw_value, Fmt::hex>{}(buf, fp.value());
    } else if constexpr (Format == Fmt::binary) {
      return Int<raw_value, Fmt::binary>{}(buf, fp.value());
    } else {
...
template <traits::FixPoint T> struct DefaultFormat<T> : public FixPoint<T> {};

template <traits::FixPoint T, Fmt Format = Fmt::normal,
  ...
  constexpr FormatResult operator()(std::span<char> buf, Enum value) const {
    if constexpr (traits::FlagEnum<Enum>) {
      ...
      for (std::size_t i = 0; i < sizeof(Enum) * 8; ++i) {
        ...
        buf = buf.subspan(name.size() + offset);
      }
      ...
      const std::string_view name = cli::ctti::enum_name(value);
...
template <traits::String T> struct DefaultFormat<T> : public String<T> {};
  ...
  constexpr FormatResult operator()(std::span<char> buf,
    ...
    for_each(
        [&error, &written, &first, &buf](const auto &field) {
          ...
          using Formatter =
              DefaultFormat<std::remove_cvref_t<typename Field::type>>;
...
template <traits::Struct T, char Assignment = '=', char MemberSeparator = ',',
          char Prefix = '{', char Postfix = '}', bool UseNames = true>
struct field_formatter_for {
  template <class... Fields>
  using type_ = FieldGroup<Assignment, MemberSeparator, Prefix, Postfix,
                           UseNames, Fields...>;
...
template <traits::Struct T, class Name = string_constant<>,
          ...
          char Postfix = '}', bool UseNames = true>
class Struct : field_formatter_for<T, Assignment, MemberSeparator, Prefix,
                                   Postfix, UseNames>::type {
  using Base = typename field_formatter_for<T, Assignment, MemberSeparator,
...
template <> struct DefaultFormat<char> : Char<> {};
