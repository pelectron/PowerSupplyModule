...
template <class Int> struct FmtIntTestVector {
  Int input;
...
template <class T> std::string max_string() {
  return std::to_string(std::numeric_limits<T>::max());
}

template <class T> std::string min_string() {
...
TEMPLATE_TEST_CASE(
    "format::Int<TestType, Fmt::normal, UseSignForPositive=false>", "[format]",
...
TEMPLATE_TEST_CASE(
    "format::Int<TestType, Fmt::normal, UseSignForPositive=true>", "[format]",
    uint32_t, uint16_t, uint8_t, int32_t, int16_t, int8_t) {
  if constexpr (std::is_signed_v<TestType>) {
    SECTION("positive values") {
      FmtIntTestVector<TestType> vectors[]{
